...
const AdoptList: React.FC = () => {
  const { axios } = useAxios()
  const [isLoading, setIsLoading] = React.useState(true)
  const [pets, setPets] = React.useState<IAdoptLits[]>([])
  ...
  const [orderBy, setOrderBy] = React.useState<keyof IAdoptLits>('nombre')
  ...
  const handleOnClickButtonView = React.useCallback(
    (id: string) => {
      navigate(`/adopt/view/${id}`)
    },
    [navigate]
  )

  const handleRequestSort = (event: React.MouseEvent<unknown>, property: keyof IAdoptLits) => {
  ...
  React.useEffect(() => {
    if (isLoading) {
      axios
        .get(process.env.REACT_APP_API_URL + 'admin/adopts')
        .then((response) => {
          const petsResponse = (response.data as IAdoptLitsResponse[]).map((elem) => elem.pet)
