...
interface IEnhancedTableHeadProps {
  numSelected: number
  onRequestSort: (event: React.MouseEvent<unknown>, property: keyof IAdoptLits) => void
  order: Order
  orderBy: string
  rowCount: number
}
...
const EnhancedTableHead: React.FC<IEnhancedTableHeadProps> = (props) => {
  ...
  const createSortHandler =
    (property: keyof IAdoptLits) => (event: React.MouseEvent<unknown>) => {
      onRequestSort(event, property)
    }
  return (
    <TableHead>
      <TableRow>
        {headCellsAdopts.map((headCell) => (
          <TableCell
            key={headCell.id}
