...
import { IHistory, IHistoryPet } from '../../resource/historyData'
...
const ListHistoryPet: React.FC = () => {
  const { axios } = useAxios()
  let { id } = useParams()
  const [isLoading, setIsLoading] = React.useState(true)
  const [petHistories, setHistories] = React.useState<IHistory[]>([])
  ...
  const [orderBy, setOrderBy] = React.useState<keyof IHistory>('createdAt')
  const [dense] = React.useState(true)
  const [page, setPage] = React.useState(0)
  const [rowsPerPage, setRowsPerPage] = React.useState(10)

  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - petHistories.length) : 0
  ...
  React.useEffect(() => {
    if (id !== undefined && isLoading) {
      axios
        ...
        .then((response) => {
          let history = response.data as IHistoryPet
          ...
          <TableContainer>
            ...
            >
              <EnhancedTableHead
                ...
                rowCount={petHistories.length}
              />
              <TableBody>
                {stableSort(
                  petHistories.map((petVacune) => ({
                    // eslint-disable-next-line @typescript-eslint/naming-convention
                    id: petVacune.id,
                    title: petVacune.title,
                    description: petVacune.description,
